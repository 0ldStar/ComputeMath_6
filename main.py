...
def graph_trapezium_method(a, b, h):
    y = []
    x = []
    xi = a
    while xi <= b:
...
# create func - broken line, that have 2 entered dotes
def line_2_dotes(x1, x2, y1, y2, x):
    return (x - x1) * (y2 - y1) / (x2 - x1) + y1


# Expand list in fields behind dotes (previously we have broken line)
...
# calculate integral for const error
def calc(power):
    n = 2
    h = (end - begin) / n
    while (
            abs(parabola_method(begin, end, h / 2) - parabola_method(begin, end, h))) > 10 ** power:
        n += 1
        h = (end - begin) / n
    print(abs(trapezium_method(begin, end, h / 2) - trapezium_method(begin, end, h)))
    print('\n10 to the ' + str(power) + ' power: ' + str(n))
    print("Result: " + str(round(parabola_method(begin, end, n), abs(power) + 1)))
    x, y = graph_trapezium_method(begin, end, h)
    err, arg = expand_list(y, x)
    print(len(y))
    print(len(err))
    X = np.arange(begin, end, h / 100)
    print(arg[0], arg[len(arg) - 1], h)
    print(X[0], X[len(X) - 1], h)
    print(len(X))
    ...
    plt.hlines(10 ** power, x[0], x[len(x) - 1], color='r', label="graph 10 ^" + str(power))
    plt.legend()
    plt.grid()
    plt.show()
...
for i in range(len(n)):
    print("n = " + str(n[i]))
    print(trapezium_method(begin, end, (end - begin) / n[i]))

print("\nParabola method:")
for i in range(len(n)):
    print("n = " + str(n[i]))
    print(parabola_method(begin, end, (end - begin) / n[i]))
